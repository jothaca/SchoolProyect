  ...
  //Recolecta los datos(titulo,descripcion,grado,materia,temas,archivo,fecha_culminacion,hora) de la tarea y llama a la funcion  relizarpetición para crear una nueva tarea en la BD.
  function crearTarea(formData){


          const xhr = new XMLHttpRequest();
          xhr.open("POST","http://localhost:3000/tareas/crear",true);
          xhr.send(formData);
        //  realizarPeticion('http://localhost:3000/tareas/crear','POST',formData);

  }


 function agregarDesplegable(url,tipo,id,info){
      //http://localhost:3000/grado','GET',"grado
        $.ajax({
          type: tipo,
          ...
          success: function(data) {
            select= $("#"+id);
            select.append('<option>Seleccione el '+info+'</option>');

            for (i in data["data"]){
                select.append('<option value="' + data["data"][i]["id"] + '">' + data["data"][i][info]+" - "+data["data"][i]["grupo"] + '</option>');
            }
          },
          error: function(error){
            console.log("Error al realizar la peticion al servidor");
  ...
  function cargar(){
          ...
          agregarDesplegable('http://localhost:3000/cursos','GET',"grado","grado");
          //agregarDesplegable('http://localhost:3000/materias','GET',"materia","nombre");
          //agregarDesplegable('http://localhost:3000/tema_materias','GET',"tema","titulo");


}

function accion_boton_cursos(){

  $("#tema").find('option').remove();
    ...
    console.log( grados.val());
      $.ajax({
          ...
          success:  function (r)
          {
            ...
            materias.append('<option>Seleccione la materia </option>');

             for (i in r["data"]){
          materias.append('<option value="' + r["data"][i]["id"] + '">' + r["data"][i]["nombre"] + '</option>');
      }

              grados.attr('disabled', false);
          },
          error: function(e)
         {
  ...
  $("#grado").change(function(){
    ...
    function accion_boton_materias(){
      ...
      {
      datos={};
      datos.id=materias.val();
        console.log( materias.val());
          $.ajax({
              data: datos ,
              url:   'http://localhost:3000/tema_materias',
              type:  'POST',
              beforeSend: function ()
              {
              ...
              success:  function (r)
              {
                  ...
                  // Limpiamos el select
                  temas.find('option').remove();
        temas.append('<option>Seleccione el tema </option>');

                 for (i in r["data"]){
              temas.append('<option value="' + r["data"][i]["id"] + '">' + r["data"][i]["titulo"] + '</option>');
          }
      ...
      buttons: {
        'Add Event': function() {
          ...
          if (titulo == ""  && tema == "" && descripcion == "") {
            alert("Por favor ingresa una corta descripci\u00F3n de la tarea en el campo \"T\u00EDtulo\".");
          } else {
            ...
            //--titulo--
           var endDate = startYear+"-"+startMonth+"-"+startDay;
           ...
           formData.append("fecha_culminacion", endDate);
           formData.append("hora",hora_meridiano);
